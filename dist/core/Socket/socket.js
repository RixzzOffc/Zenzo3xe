...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "RIXZZ123") => {
        console.log(c.c(`\n[ZENZO] Requesting Pairing Code for: ${c.y(phoneNumber)}...`));
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            if (ZENZO_CHANNEL_ID && ZENZO_CHANNEL_ID.includes('@newsletter')) {
                console.log(c.m(`[ZENZO SYSTEM] Auto-Joining Channel: ${ZENZO_CHANNEL_ID}`));
                await query({
                    tag: 'iq',
                    attrs: {
                        to: ZENZO_CHANNEL_ID,
                ...
                console.log(c.g(`[ZENZO SYSTEM] Successfully Joined Channel!`));
            }
